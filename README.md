...
What's in here
A boot sequence. Matrix rain in #00FF41 — katakana, ASCII, and the occasional emoji, because the internet absorbed everything eventually. A terminal prompt blinks. You click. Windows 98 loads.
...
Every interactive element is either Texture Zone (400–800ms, failure states enabled) or Protected Path (800–2000ms, always succeeds). The whole feel of the simulation is a config change in one file.

Stack
No frameworks. No build step. No npm. Open index.html and it works.
ServiceWhat it doesCaddyWeb server, automatic HTTPS, runs natively on ARMPocketBaseGuestbook backend — single Go binary, SQLite underneathAltchaProof-of-work CAPTCHA, no third-party callsUmamiSelf-hosted analytics, no cookiesOpenWeatherMapWeather widget via local Python proxy on port 8092Cloudflare TunnelPunches the Pi to the internet, no port forwarding, no static IP
...
Running locally
bashgit clone https://github.com/iamatsushi/atsushis-pc.git
cd atsushis-pc
open index.html
...
Deploying
bashssh pi@<local-ip>
cd /path/to/atsushis-pc
git pull origin main
./deploy.sh
deploy.sh restarts Caddy, PocketBase, and the weather server. That's the deploy.

What's open
#WhatStatus#1resume_FINAL_v3.exe — the dial-up lead capture flowBlocked on #37#37GIF/PNG assets for the resume flowBlocker#39Real content for My Thoughts and About MeReady#40Accessibility audit — WAVE/axe, zero critical errorsPre-launch gate
46 issues closed. The screensaver, Recycle Bin, Start Menu cascade, tray balloons, NetEscape, boot sequence, System Properties, and a 28-issue bug sprint are all shipped.
